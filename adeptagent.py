...
class AdeptAgent:
    ...
    def move_rel_tool(self, pose, break_move=True):
        ...
        self.client.send_msg("move_rel_tool:%s" % pose_str)
        if break_move:
            self.client.send_msg("break")
        
    def move_rel_world(self, pose, break_move=True):
        pose_str = vec_to_str(pose)
        self.client.send_msg("move_rel_world:%s" % pose_str)
        if break_move:
            self.client.send_msg("break")
    ...
    def move_rel_frame(self, fname, pose, break_move=True):
        ...
        cmd = vec_to_str(vec)
        self.client.send_msg("move_rel_frame:%s" % cmd)
        if break_move:
            self.client.send_msg("break")
    ...
    def move_to(self, pose, break_move=True):
        pose_str = vec_to_str(pose)
        self.client.send_msg("move_to:%s" % pose_str)
        if break_move:
            self.client.send_msg("break")
    ...
    def get_tool(self, tool_id):
        self.client.send_msg("get_tool:%d" % tool_id)
